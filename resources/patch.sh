...
Patch_Volume()
{
	...
	if [[ $model_metal == *$model* ]]; then
		echo -e ${text_warning}"! Metal graphics card compatible model detected."${erase_style}
		echo -e ${text_message}"/ These graphics patches are not for Metal cards."${erase_style}
		echo -e ${text_message}"/ Input an operation number."${erase_style}
		echo -e ${text_message}"/     1 - Install the graphics patches"${erase_style}
		echo -e ${text_message}"/     2 - Continue without the graphics patch"${erase_style}
		Input_On
	...
	echo -e ${text_progress}"> Patching graphics drivers."${erase_style}
	...
	echo -e ${move_up}${erase_line}${text_success}"+ Patched graphics drivers."${erase_style}


	echo -e ${text_progress}"> Patching audio drivers."${erase_style}
	...
	echo -e ${move_up}${erase_line}${text_success}"+ Patched audio drivers."${erase_style}


	echo -e ${text_progress}"> Patching backlight drivers."${erase_style}
	...
	echo -e ${move_up}${erase_line}${text_success}"+ Patched backlight drivers."${erase_style}


	echo -e ${text_progress}"> Patching ambient light sensor drivers."${erase_style}

		cp -R "$resources_path"/AmbientLightSensorHID.plugin "$volume_path"/System/Library/Extensions/AppleSMCLMU.kext/Contents/PlugIns/

	echo -e ${move_up}${erase_line}${text_success}"+ Patched ambient light sensor drivers."${erase_style}


	echo -e ${text_progress}"> Patching AirPort drivers."${erase_style}
	...
	echo -e ${move_up}${erase_line}${text_success}"+ Patched AirPort drivers."${erase_style}


	if [[ $volume_version_short == "10.15" ]]; then
		echo -e ${text_progress}"> Patching Ethernet drivers."${erase_style}
		...
		echo -e ${move_up}${erase_line}${text_success}"+ Patched Ethernet drivers."${erase_style}
	fi


	if [[ $model == "MacBook4,1" ]]; then
		echo -e ${text_progress}"> Patching Bluetooth drivers."${erase_style}
		...
		echo -e ${move_up}${erase_line}${text_success}"+ Patched Bluetooth drivers."${erase_style}
	fi


	if [[ $volume_version == "10.14."[4-6] || $volume_version_short == "10.15" ]]; then
		echo -e ${text_progress}"> Patching Siri application."${erase_style}
		...
		echo -e ${move_up}${erase_line}${text_success}"+ Patched Siri application."${erase_style}
	fi


	if [[ $volume_version_short == "10.15" ]]; then
		echo -e ${text_progress}"> Patching IDE drivers."${erase_style}

			cp -R "$resources_path"/AppleIntelPIIXATA.kext "$volume_path"/System/Library/Extensions/IOATAFamily.kext/Contents/PlugIns

		echo -e ${move_up}${erase_line}${text_success}"+ Patched IDE drivers."${erase_style}
	fi


	echo -e ${text_progress}"> Patching software update check."${erase_style}
	...
	echo -e ${move_up}${erase_line}${text_success}"+ Patched software update check."${erase_style}
	...
	if [[ $volume_version_short == "10.1"[4-5] ]]; then
		echo -e ${text_progress}"> Patching kernel panic issue."${erase_style}

			Output_Off rm -R "$volume_path"/System/Library/UserEventPlugins/com.apple.telemetry.plugin

		echo -e ${move_up}${erase_line}${text_success}"+ Patched kernel panic issue."${erase_style}
	fi
...
Input_Operation_APFS()
{
	if [[ "$(diskutil info "$volume_name"|grep "APFS")" == *"APFS"* ]]; then
		if [[ $model_apfs == *$model* ]]; then
			echo -e ${text_warning}"! APFS incompatible model detected."${erase_style}
			...
			if [[ $operation_apfs == "2" && $volume_patch_apfs == "1" ]]; then
				echo -e ${text_warning}"! The APFS patch is already installed."${erase_style}
				echo -e ${text_message}"/ Run the restore tool to remove it."${erase_style}
			fi
		fi
	fi
}
	...
	if [[ "$(diskutil info "$volume_name"|grep "APFS")" == *"APFS"* ]]; then
		echo -e ${text_progress}"> Patching Preboot partition."${erase_style}
			...
			if [[ ! "$(diskutil info "${preboot_identifier}"|grep "Volume Name")" == *"Preboot" ]]; then
				echo -e ${text_error}"- Fatal error patching Preboot partition."${erase_style}
		...
		echo -e ${text_progress}"> Patching Recovery partition."${erase_style}
			...
			if [[ ! "$(diskutil info "${recovery_identifier}"|grep "Volume Name")" == *"Recovery" ]]; then
				echo -e ${text_warning}"! Error patching Recovery partition."${erase_style}
			else
				...
				if [[ ! "$recovery_folder" == "/Volumes/Recovery/"* ]]; then
					...
					Output_Off sed -i '' 's|\immutablekernel|\prelinkedkernel|' "$recovery_folder"/com.apple.boot.plist
				...
				echo -e ${move_up}${erase_line}${text_success}"+ Patched Recovery partition."${erase_style}
			fi
		fi
	fi
